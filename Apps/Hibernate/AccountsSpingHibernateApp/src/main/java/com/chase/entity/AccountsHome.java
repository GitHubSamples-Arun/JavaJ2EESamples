...
// default package
// Generated May 7, 2017 12:57:00 AM by Hibernate Tools 5.2.1.Final
...
public class AccountsHome {
	...
	public Accounts merge(Accounts detachedInstance) {
		...
		try {
			Accounts result = (Accounts) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Accounts findById(java.math.BigDecimal id) {
		log.debug("getting Accounts instance with id: " + id);
		try {
			Accounts instance = (Accounts) sessionFactory.getCurrentSession().get("Accounts", id);
			if (instance == null) {
	...
	public List findByExample(Accounts instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Accounts").add(Example.create(instance))
					.list();
